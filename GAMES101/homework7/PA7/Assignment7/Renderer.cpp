...
void Renderer::Render(const Scene& scene)
{
    ...
    int spp = 512;
    Vector3f l = (8.0f * Vector3f(0.747f+0.058f, 0.747f+0.258f, 0.747f) + 15.6f * Vector3f(0.740f+0.287f,0.740f+0.160f,0.740f) + 18.4f *Vector3f(0.737f+0.642f,0.737f+0.159f,0.737f));
    std::cout << "SPP: " << spp << "\n";
                ...
                for (int xIndex = 0; xIndex < ssaaTime; xIndex++)
                {
                    for (int yIndex = 0; yIndex < ssaaTime; yIndex++)
                    {
                        for (int sampleTime = 0; sampleTime < checkTime; sampleTime++)
                        {
                            ...
                            Vector3f sampleDir = normalize(Vector3f(-sampleX, sampleY, 1));
                            Vector3f result = scene.castRay(Ray(eye_pos, sampleDir), 0);
                            if (result == l)
                                count++;
                            result = clamp(Vector3f(0), Vector3f(1), result);
                            framebuffer[m] += result / spp;
                            if (count >= spp / 2)
                            {
                                framebuffer[m] = l;
                                break;
