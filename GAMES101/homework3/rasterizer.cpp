...
//         {
//             if (insideTriangle(float(i) + 0.5f, float(j) + 0.5f, t.v))
//             {
...
//                     set_pixel({float(i), float(j)}, pixel_color);
//                     depth_buf[index] = zp;
//                 }
//             }
//         }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int i = xMin; i < xMax; i++)
    {
        for (int j = yMin; j < yMax; j++)
        {
            ...
            {
                float x = (float)i + diff[k].first, y = (float)j + diff[k].second;
                if (insideTriangle(x, y, t.v))
                {
                    ...
                    float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                    zp *= Z;
                    int index = getSSAAindex(i, j, k);
                    if (ssaaDepthBuffer[index] > zp)
                    {
                        auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2], 1);
                        auto interpolated_normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], 1);
                        auto interpolated_texcoords = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2], 1);
                        ...
                        payload.view_pos = interpolated_shadingcoords;
                        auto pixel_color = fragment_shader(payload);
                        ssaaFrameBuffer[index] = pixel_color;
                        ssaaDepthBuffer[index] = zp;
                    }
                }
            }
        }
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    ...
    ssaaFrameBuffer.resize(w * h * 4 * 4);
    ssaaDepthBuffer.resize(w * h * 4 * 4);
    texture = std::nullopt;
}
...
int rst::rasterizer::getSSAAindex(int x, int y, int i)
{
    return ((height - y - 1) * width + x) * 4 + i;
}
...
void rst::rasterizer::setSSAAframeBuff()
{
    ...
    for (int x = 0; x < width; x++)
    {
        for (int y = 0; y < height; y++)
        {
            for (int i = 0; i < 4; i++)
            {
                int index = getSSAAindex(x, y, i);
                Col += ssaaFrameBuffer[index];
            }
            p << x, y;
            set_pixel(p, Col / 4);
            Col << 0, 0, 0;
